...
louisfile(x) = sprintf('/nvme/correlation_audio_data/louis_novariety/C1--cd-C1--%05i.powerspec',x)
chirpsfile(x) = sprintf('/nvme/correlation_audio_data/chirpsandclaps/C4--cd-C1--%05i.powerspec',x)
banjofile(x) = sprintf('/nvme/correlation_audio_data/banjo/C4--cd-C1--%05i.powerspec',x)
GNUTERM = "qt"
set term png size 1800,1200
set output 'plotting.louis_novariety_full.png'
#set output 'plotting.chirps.png'
#set output 'plotting.banjo.png'
set xlabel 'time [arb]'
set xrange [0:2600]
#set xrange [1700:1800]
set ylabel 'frequency [Hz]'
set cbrange [1e4:1e8]
set yrange [0:2e3]
...
sz=100
splot for [i=0:25] louisfile(i) mat u ($1+i*sz):($2*10):3 notitle
#    EOF
