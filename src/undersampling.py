...
def main():
    ...
    for batch in range(nbatches):
        ...
        data = 1e3*raw.reshape(ninstances,sz).T
        ...
        DC_up = np.zeros((frq_.shape[0]*upscale,DC_.shape[1]),dtype=float)
        DC_up[:frq_.shape[0],:] = DC_[:frq_.shape[0],:]
        ...
        logic = (dsc*dcc)[:frq.shape[0]//2,:]
        logic_ = (dsc_*dcc_)[:frq_.shape[0]//2,:]
        logic_up = (dsc_up*dcc_up)[:frq_.shape[0]//2*upscale,:]
        np.savetxt('%s/%s_b%i_sample.dat'%(path,fname,batch),d,fmt='%.3f')
        ...
        np.savetxt('%s/%s_b%i_logic.dat'%(path,fname,batch),logic,fmt='%.3f')
        np.savetxt('%s/%s_b%i_logic_.dat'%(path,fname,batch),logic_,fmt='%.3f')
        np.savetxt('%s/%s_b%i_logic_up.dat'%(path,fname,batch),logic_up,fmt='%.3f')
        #edgeslist = findedges(logic,-100)
        #edgeslist_ = findedges(logic_,-100)
        #edgeslist_up = findedges(logic_up,-5)
        f = open('%s/%s_b%i_logic_edges.dat'%(path,fname,batch),'w')
        f_up = open('%s/%s_b%i_logic_up_edges.dat'%(path,fname,batch),'w')
        ...
        f.close()
        f_up.close()
    #np.savetxt('%s/%s_logic_compare.out'%(path,fname),np.column_stack(out),fmt='%.3f')
    f = open('%s/%s_logic_compare_upscale%i.dat'%(path,fname,upscale),'w')
    _ = [f.write('%.4f\t%.4f\n'%(p[0],p[1])) for p in out]
    f.close()
    print(out[:5][:])
    hbins = np.arange(-10,10,0.02)
    h = np.histogram(np.array(out)[:,1]-np.array(out)[:,0],hbins)[0]
    np.savetxt('%s/%s_logic_compare_upscale%i.hist'%(path,fname,upscale),np.column_stack((hbins[:-1],h)))
    return
